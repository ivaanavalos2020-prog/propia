...
export default function LoginPage() {
  ...
  // Google OAuth
  const [googleEstado, setGoogleEstado] = useState<'idle' | 'cargando' | 'error'>('idle')
  ...
  async function handleGoogle() {
    ...
    if (error) {
      setGoogleError('No se pudo conectar con Google. Intentá de nuevo.')
      setGoogleEstado('error')
    }
    // Sin error: el navegador redirige, quedamos en 'cargando'
  }
      ...
      <div className="flex flex-1 items-center justify-center px-4 py-12">
        <div className={`w-full max-w-sm transition-all duration-500 ease-out ${visible ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-95 translate-y-4'}`}>
          ...
          <div className="mb-6 text-center">
            ...
            <h1 className="text-2xl font-extrabold tracking-tight text-slate-900 tracking-propia">
              Ingresá a PROPIA
            </h1>
          </div>
          ...
          <div className="rounded-2xl border border-slate-300 bg-white p-6 shadow-sm">
            ...
            {modoReset ? (
              ...
              <>
                {/* Google */}
                <button type="button" onClick={handleGoogle} disabled={googleEstado === 'cargando'}
                  className="flex w-full items-center justify-center gap-3 rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 transition-colors hover:border-slate-400 hover:bg-slate-50 disabled:opacity-50">
                ...
                {googleError && <p className="mt-2 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-600">{googleError}</p>}

                {/* Separador */}
                <div className="my-5 flex items-center gap-3">
                  <div className="flex-1 h-px bg-slate-200" />
                  <span className="text-xs text-slate-400 font-medium whitespace-nowrap">o ingresá con tu email</span>
                  <div className="flex-1 h-px bg-slate-200" />
                </div>
